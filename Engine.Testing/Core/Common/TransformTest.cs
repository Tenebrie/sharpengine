...
public class TransformTest
{
    ...
    {
        Assert.Equal(new Vector3(0, 0, 0),   Transform.Identity.Position);
        
        Assert.Equal(new Vector3(3, 5, 8),   Transform.FromTranslation(3, 5, 8).Position);
        Assert.Equal(new Vector3(3, 5, 8),   Transform.Identity.TranslateGlobal(3, 5, 8).Position);
        Assert.Equal(new Vector3(6, 10, 16), Transform.Identity.TranslateGlobal(3, 5, 8).TranslateGlobal(3, 5, 8).Position);
    }
    ...
    {
        Assert.Equal(new Vector3(1, 1, 1), Transform.Identity.Scale);
        
        Assert.Equal(new Vector3(3, 5, 8),   Transform.FromScale(3, 5, 8).Scale);
        Assert.Equal(new Vector3(3, 5, 8),   Transform.Identity.Rescale(3, 5, 8).Scale);
        Assert.Equal(new Vector3(9, 25, 64), Transform.Identity.Rescale(3, 5, 8).Rescale(3, 5, 8).Scale);
    }
    ...
    public void AppliesMultipleOperationsCorrectly()
    {
        ...
        Assert.Equal(new Vector3(10, 100, 10), transform.Position);
    ...
    public class Basis
    {
        ...
        public void GetsValidBasis()
        {
            ...
            VectorAssert.Equal(new Vector3(1, 0, 0), identityBasis.XAxis);
            VectorAssert.Equal(new Vector3(0, 1, 0), identityBasis.YAxis);
            VectorAssert.Equal(new Vector3(0, 0, 1), identityBasis.ZAxis);
            
            var pitchUpBasis = Transform.FromRotation(90, 0, 0).Basis;
            VectorAssert.Equal(new Vector3(1, 0, 0), pitchUpBasis.XAxis);
            VectorAssert.Equal(new Vector3(0, 0, -1), pitchUpBasis.YAxis);
            VectorAssert.Equal(new Vector3(0, 1, 0), pitchUpBasis.ZAxis);
            
            var yawRightBasis = Transform.FromRotation(0, 90, 0).Basis;
            VectorAssert.Equal(new Vector3(0, 0, 1), yawRightBasis.XAxis);
            VectorAssert.Equal(new Vector3(0, 1, 0), yawRightBasis.YAxis);
            VectorAssert.Equal(new Vector3(-1, 0, 0), yawRightBasis.ZAxis);
            
            var rollRightBasis = Transform.FromRotation(0, 0, 90).Basis;
            VectorAssert.Equal(new Vector3(0, -1, 0), rollRightBasis.XAxis);
            VectorAssert.Equal(new Vector3(1, 0, 0), rollRightBasis.YAxis);
            VectorAssert.Equal(new Vector3(0, 0, 1), rollRightBasis.ZAxis);
        }
        
        [Fact]
        public void RotatesAroundSingleAxis()
        {
            var pitchTransform = Transform.Identity.RotateAroundGlobal(Vector3.Pitch, 90);
            var yawTransform = Transform.Identity.RotateAroundGlobal(Vector3.Yaw, 90);
            var rollTransform = Transform.Identity.RotateAroundGlobal(Vector3.Roll, 90);
        ...
        public void RotatesAroundGlobalAxis()
        {
            var transform = Transform.Identity
                .RotateAroundGlobal(Vector3.Pitch, 90)
                .RotateAroundGlobal(Vector3.Yaw, 90)
                .RotateAroundGlobal(Vector3.Roll, -90);
        ...
        {
            var transform = Transform.Identity
                .RotateAroundLocal(Vector3.Pitch, 90)
                .RotateAroundLocal(Vector3.Yaw, 90)
                .RotateAroundLocal(Vector3.Roll, 90);
            QuatAssert.Equal(QuatMakers.FromRotation(0, 90, 0), transform.Rotation);
        }
    }
}
